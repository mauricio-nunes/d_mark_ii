from ..db.repositories.users_repo import (
    get_user_by_username, update_login_attempts, update_password
)
...
def login_flow() -> dict | None:
    ...
    if check_password(raw, user["password_hash"]):
        # reset tentativas
        update_login_attempts(user["id"], 0, 0)
        if user["must_change_password"]:
            ...
            if new1 != new2 or not new1:
                ...
                return None
            update_password(user["id"], hash_password(new1), must_change=False)
            print("Senha alterada com sucesso.\n")
        return dict(user)
    else:
        tentativas = user["tentativas"] + 1
        bloqueado = 1 if tentativas >= MAX_ATTEMPTS else 0
        update_login_attempts(user["id"], tentativas, bloqueado)
        if bloqueado:
