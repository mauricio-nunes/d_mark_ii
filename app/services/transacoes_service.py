...
def incluir(data: dict) -> int:
    """
    data: {data, tipo, corretora_id?, quantidade(str), ticker(int), carteira_id(int),
           preco_unitario?, taxas?, observacoes?}
    Regras:
      - valida data, tipo, existência de ticker/carteira
      - VENDA/TRANSFERENCIA (saída) não pode exceder posição até a data
      - TRANSFERENCIA 'entrada' deve ser incluída via função transferir()
    """
    ...
    if tipo in ("VENDA","TRANSFERENCIA"):
        # trata como saída (se vier sem preco_unitario)
        _can_sell_or_transfer(data["ticker"], data["carteira_id"], data["data"], data["quantidade"])

    # bonificação: força preco_unitario=0, sem taxas
    if tipo == "BONIFICACAO":
        data["preco_unitario"] = "0"
        data["taxas"] = "0"

    # subscrição/compra: aceita taxas (entram no PM). Venda: taxas ignoradas no PM.
    return transacoes_repo.create(data)

def editar(tid: int, data: dict) -> None:
    # Para simplificar, aplicamos mesma validação da inclusão
...
def transferir(data_base: str, ticker_id: int, origem_id: int, destino_id: int, quantidade: str) -> tuple[int,int]:
    ...
    Cria duas transações:
      - SAÍDA (origem): tipo=TRANSFERENCIA, preco_unit=0 (só reduz)
      - ENTRADA (destino): tipo=TRANSFERENCIA, preco_unit=PM da ORIGEM na data_base
    """
    if origem_id == destino_id: raise ValidationError("Origem e destino não podem ser a mesma carteira.")
    _parse_date(data_base)
    _exists(ativos_repo, ticker_id, "Ativo"); _exists(carteiras_repo, origem_id, "Carteira origem"); _exists(carteiras_repo, destino_id, "Carteira destino")
    _can_sell_or_transfer(ticker_id, origem_id, data_base, quantidade)

    # PM da origem até a data
