...
def _render(rows, page, pages, total):
    ...
    for r in rows:
        print(f"{r['id']:<5} {r['data']:<10} {r['tipo']:<14} {r['ticker_str']:<8} {r['carteira_str']:<10} "
              f"{r['quantidade']:>12} {str(r.get('preco_unitario') or ''):>12} {str(r.get('taxas') or ''):>10}")
...
def _coleta_campos(reg=None):
    ...
    if not reg:
        ...
        # função lista de corretoras já existe como list_corretoras
...
def tela_transacoes():
    ...
    while True:
        ...
        start, _ = pager.range()
        rows = repo.list(texto=filtro, ticker_id=ticker_id, carteira_id=carteira_id,
                         offset=start, limit=pager.page_size, apenas_ativas=True)
